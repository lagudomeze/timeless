...
| **时钟** | 世界现在停不停 | `clock` 域；`Time<Virtual>` 只由 `process_pause_requests` 写 |
...
时钟**不属于任何领域**：它不认识决策槽、不认识威胁、不认识按键，只认识
「请求 → 集合 → 时钟」。谁有理由停表，谁就按自己的**事实**写一条请求。

```text
frozen ⟺ PauseReasons 非空 || ManualPause
```

- `PauseRequest::Pause(reason)` 是**断言式**：谁这一帧还想让世界停着就写一条；
  下一帧不再断言，原因自然消失，**不需要谁去撤销**（各领域走这条）。
- `PauseRequest::Toggle` 是**翻转**：冻着就**清空原因集合**（世界立刻动）、
  没冻就置 `ManualPause`。输入域只发一条 `Toggle`，**不读任何状态**。
- **`Toggle` 不带原因**：玩家自己按的那个暂停不进 `PauseReasons`——那个集合回答的是
  "**别人**为什么在停表"（HUD 展示用），玩家的开关是**状态**不是原因，记在
  `ManualPause` 这个布尔上。这就是删掉 `LatchedReasons` 的根据：那个闩存在的唯一
  理由就是"记住玩家开过手动暂停"，而时钟自己（`Time<Virtual>`）就是那个状态。
- **为什么分成两种时序**：断言式原因每帧都要重新声明，而玩家按键是**一次性事件**。
  共用一条消息就得先猜"上一帧有没有人断言过这个原因"——而集合里同时躺着别人的
  原因，猜不准。曾经的 bug 就是从 `PauseReasons` 反推手动暂停：威胁冻着时集合里
  只有 `THREAT`，于是把"继续"误判成"暂停"，按下去反而又停一层。
- **落地顺序是语义的一部分**（`clock::process_pause_requests`，帧末 `ClockSet`）：
  ① 先收本帧断言重建集合 → ② 再处理 `Toggle`（判据是**入帧时冻着吗**，不是
  "手动开关开着吗"；翻成"继续"时**当场清空集合**）→ ③ 落到时钟。
  顺序确定：**同一帧里仍然成立的原因会照常加回来**（比如"还等着你决策"），
  所以"放开世界"不等于"跳过决策"。

当前的原因：`AWAITING`（PC 还没决定）/ `THREAT`（PC 被威胁且还没表态）。

> 引用常量一律用 `clock::AWAITING` / `clock::THREAT`，**不要硬编码字符串**。
> （M23 之前叫 `SLOT_EMPTY`；改名是因为判据从"槽空不空"变成了"决定了没有"。）

**唯一**写 `Time<Virtual>` 的地方是帧末 `ClockSet` 的 `process_pause_requests`。
Bevy 每帧把虚拟时间拷进通用 `Time`，因此位移、投射物、`Lifetime`、后摇计时全部
自动停表，各域不需要任何 `if paused` 分支。
