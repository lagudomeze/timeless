...
| 铁律 | 违反的后果 |
| :--- | :--- |
| 一个域 = 一个目录 = 一个 `Plugin` | 领域边界消失，循环依赖 |
| 输入只翻译、不执行 | 输入系统变成上帝系统，`KeyCode` 渗进各领域 |
| 消息与消费系统同域 | 生产者与消费者绑死，换触发源要动两个领域 |
| 行动实体化 | 调度器开始认识载荷，"新增动作不改调度器"当场失效 |
| 暂停只用 `Time<Virtual>` | 各领域冒出 `if paused` 分支，冻结时机再也说不清 |
| 组件写入者唯一 | 同一份状态两处写，谁覆盖谁要靠运气（HUD 面板撞过一次） |
| 决策按格、结算按真实距离 | 两套坐标各算一半，命中位置飘忽 |
| 组装单向依赖 | 改角色配置会波及战斗规则 |
| 功能不是领域 | 每个功能都长出一个空领域 |
