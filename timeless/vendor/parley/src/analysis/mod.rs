...
impl AnalysisDataSources {
    ...
    fn word_segmenter(&self) -> WordSegmenterBorrowed<'static> {
        // 本地补丁（bevy 0.19 / parley 0.9.0）：
        // 原版用 new_for_non_complex_scripts，不加载 CJK 词典，
        // 中文/日文会触发 icu_segmenter 的 "No segmentation model" 刷屏。
        // 改用 new_auto（需 icu_segmenter 的 compiled_data + auto feature，
        // 由 timeless-app 直接依赖统一启用）。
