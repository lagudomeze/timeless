...
//!    日志正文会变豆腐块）；
//! 4. **区域根节点声明“吃掉指针”**：`FocusPolicy::Block` + `RelativeCursorPosition`。
//!    指针压在某个区域上时，`interaction` 域既不拾取也不翻译点击（见
//!    [`crate::interaction::ui_capture`]）——否则点一下日志标题会顺手把人走一格。
//!    **根节点不声明**：它是全屏容器，一旦声明，整屏都算压在 UI 上。
//!
