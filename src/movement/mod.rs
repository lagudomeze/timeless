...
//!
//! **可行走性**在 [`rules`]（纯规则：只吃两格高度差）；**站立高度**来自
//! `world::storage::ground`，它**看真实体素**——所以玩家堆的方块真的能站上去、
//! 真的挡路（建造因此不再是纯装饰）。
//!
//! 区块未加载时退回噪声地表：那条兜底让"单位贴地"在流式范围外仍然成立。
//!
...
pub use actions::{
    DASH_CELLS, DASH_COST, DASH_SPEED, DASH_TIMING, DashAction, JUMP_TIMING, JumpAction, Jumping,
    MOVE_TIMING, MoveAction, ROLL_TIMING, RollAction, dash_action_executor_system,
    dash_action_scene, declare_dash_system, declare_jump_system, declare_move_system,
...
pub use events::{DashCommand, JumpCommand, MoveCommand, MoveToCommand};
