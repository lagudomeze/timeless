...
- [x] **可行走性判定**（本次）：移动此前**完全没有**可行走性检查——点哪走哪、
      ...
      **玩家把地形堆高之后**。
      ...
      前两条**验过不是空跑**（拿掉检查后转红）。
- [ ] **单位贴地与体素碰撞（真正的部分仍未做）**：单位站在**噪声地表**上，
      **玩家放的方块完全不影响站立高度**——实测：连放 3 块石头，`ground_position`
      纹丝不动。所以现在的建造是"纯装饰"，走上去也不会站到方块上；
      可行走性也只比较噪声高度，看不到方块堆出的墙。
      真做需要让高度查询也读体素，而那会打破"地形高度是纯函数、不依赖区块加载"
      这条被 `docs` 明确记录的性质——**这是一个设计决定，得你定**：
      要么接受"贴地依赖区块已加载"，要么引一层"高度缓存/覆盖表"。
...
- [ ] **多敌人战斗：面板扩成 N 行**：现在只画得下一个敌人（显示最近的那个）。
      真要"每个敌人都有一行"得改 `unit_panel` 的布局与 `[Option<UnitRow>; 2]`
      的固定两格——**属于表现设计**，等需要时再做。
