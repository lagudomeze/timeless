...
- [x] **`ActionTemplate` 资产图 —— 审计结论：现在不建**（本次）：
      条目原本要的是"动作的静态定义（相位 / 消耗 / 效果 / 可取消规则）**资产化**，
      按**边条件在图上转移**"。逐项查过，**前四样已经都在**，第五样**没有消费者**：
      | `ActionTemplate` 想说 | 现在由谁回答 |
      | :--- | :--- |
      | 相位 | `ActionTiming { windup, recovery }`（**固定三段**：前摇 → 执行 → 后摇） |
      | 消耗 | `AbilityDef.cost: ResourceCost`（花哪条线、花多少，一处真相） |
      | 效果 | `AbilityDef.power` + **各域的载荷组件**（`MoveAction` / `FireballAction`…） |
      | 可取消规则 | **两个独立的轴**：`CombatTags`（能不能被**打断 / 招架 / 格挡**）与 `Uncancellable`（能不能被**撤销**） |
      | **按边条件在图上转移** | **没有**——每条行动只有一个 `execute_at`、固定三段，**没有任何分支** |
      **而且"资产化"这一半也已经落地**：`config/actions.ron` 把 9 个动作的
      前摇 / 后摇 / 打断抗性 / 消耗 / 威力 / 帧全外置了（见「动作数值外置」），
      热重载也能改。所以剩下的只有"图"这一层，而它要的**边条件**来自
      "一条行动打多个落地时刻 / 按条件跳阶段"——那正是 `PendingHit` 的触发条件
      （多段技能 / 蓄力两段），而 `PendingHit` 的审计结论同样是"现在不建"。
      **先造一张没有边的图，就是为假想的需求设计**（`CLAUDE.md` / `docs/skills.md`
      第七节的"够用就不加机制"）——所以这条**主动不做**。
      **触发条件（满足任一条就该做）**：
      ① 出现**多段 / 分阶段**技能（连击三下、蓄力两段、引导型法术）——
      一条行动需要多个 `execute_at` 或"到某阶段才能进下一段"；
      ② 动作的**效果要按条件转移**（"命中就追击、落空就收招"这类分支）；
      ③ 动作定义需要**工具生成 / 图形化编辑**（那时"图"才是对的表示）。
      届时最小形态：`Phase { duration, on_enter, on_exit }`（`docs/skills.md` 第七节
      已写好形状）+ 让 `ScheduledAction` 支持多个落地时刻——**不需要新调度器**。
      **顺带修掉审计查出的注释漂移**：冲刺 / 翻滚的注释写着"蹬出去**收不回来**"，
      但它们**没有** `Uncancellable`（只有跳跃有）——那两个是可以撤销的，
      "收不回来"说的是"**起手后不再受打断**"（`COMMITTED`）。两条注释已改成
      区分「不能被打断」与「不能被撤销」这两个轴。
...
- [x] **洞察力 / 时间轴悬停读数**（设计稿 ①②③ 落地）：滑到时间轴的色块上，
      ...
      **画威胁格已落地**（本次）：`Threatens.cells` / 飞行中投射物的 `TargetCell`
      现在画在地面上（`presentation/threat_grid.rs`，只读、池化 8 格）。
      住在表现层与命中特效同级，不抢鼠标高亮的通道（两套独立视觉）。
      验收：`presentation::threat_grid` 五条；**实机**（BRP）：池里 2 个可见、
      世界坐标 `(3.0,1.0)`/`(5.0,1.0)` 正是敌人火球轨迹的格中心。
      **未做**：敌人面板展开式洞察力面板（设计稿取"乙"时主动放弃的那一项）。
...
- [x] **战斗日志：谁打的谁**（本次）：日志原本只写"玩家 受到 12 点伤害"，
      ...
      **时间戳已补**（本次）：`DamageEvent.at` / `DeathEvent.at` 带上**虚拟时刻**
      （由写方在结算那一帧从 `Time<Virtual>` 取；死亡继承致命一击的时刻，
      两行因此带同一个戳），日志每行前缀 `[1.2s]` 就是"哪个时刻命中了谁"。
      验收：`a_line_is_stamped_with_the_moment_it_happened`（日志单元）与
      `a_hit_is_logged_at_the_moment_it_landed`（整机：空转 1s 后出手，前缀必须 ≥ 1.0;
      **验过不是空跑**：写方改成 `at: 0.0` 后转红，报 `[0.0s]`）。
      验收：新增 5 条日志用例（**验过不是空跑**：把出手方抹掉后
